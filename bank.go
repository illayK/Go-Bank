...
var (
	...
	commandNotFound string = color("Command not found.", "cyan+bu")
	accountNotFound string = color("Account not found.", "cyan+bu")
)
...
func signUp() {
	...
	for lineNum < numLines && isUsernameFree {
		...
		if username == decodeAccount(accountLine).Username {
			...
			fmt.Println(ansi.Color("This username is taken.", "cyan+bu"))
			main()
		}
		lineNum++
	}
	...
	fmt.Scanln(&password)
	if len(password) < 8 {
		fmt.Println(ansi.Color("Password need to be more than 8 charcters.", "cyan+bu"))
		main()
	}
...
func logIn() {
	if bothFilesEmt {
		fmt.Println(color("Error: file is empty.", "cyan+bu"))
		main()
	}
	var (
		username     string
...
func withdraw() {
	...
	fmt.Scanln(&amount)
	if amount == 0 {
		fmt.Println(color("Amount to withdraw wasn't entered.", "cyan+bu"))
		loggedIn()
	} else if amount < 0 {
		fmt.Println(color("Not valid amount.", "cyan+bu"))
	}
...
func deposit() {
	...
	fmt.Scanln(&amount)
	if amount == 0 {
		fmt.Println(color("Amount to deposit wasn't entered.", "cyan+bu"))
		loggedIn()
	} else if amount < 0 {
		fmt.Println(color("Not valid amount.", "cyan+bu"))
	}
...
func bankTransfer() {
	...
	fmt.Scanln(&amount)
	if amount == 0 {
		fmt.Println(color("You didn't enter amount to transfer.", "cyan+bu"))
		loggedIn()
	} else if amount < 0 {
		fmt.Println(color("Not valid amount.", "cyan+bu"))
		loggedIn()
	} else if amount < userLogged.Balance {
		fmt.Println(color("You don't have enough money.", "cyan+bu"))
		loggedIn()
	}
...
func updateFile(userToUpdate ...accountS) {
	...
	fileToUpdate, err := os.OpenFile(unUsedFile, os.O_RDWR, 0644)
	isErr(err)
...
func checkUsedFile() {
	...
	isErr(err)
	info2, err := os.Stat("accounts1")
	isErr(err)
	if info.Size() > 0 {
		usedFile = "accounts"
		unUsedFile = "accounts1"
